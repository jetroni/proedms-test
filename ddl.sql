...
alter table `sakila`.`store`
  add constraint `fk_store_staff`
    ...
    references `sakila`.`staff` (`staff_id`);
alter table `sakila_test`.`city`
  add constraint `fk_city_country`
    foreign key (
      `country_id`,
      `country_id`
    )
    references `sakila_test`.`country` (
      `country_id`,
      `country_id`
    );
alter table `sakila_test`.`film`
  add constraint `fk_film_language`
    ...
    references `sakila_test`.`language` (
      `language_id`,
      `language_id`
    );
alter table `sakila_test`.`film`
...
alter table `sakila_test`.`film_category`
  add constraint `fk_film_category_category`
    ...
    references `sakila_test`.`category` (
      `category_id`,
      `category_id`
    );
alter table `sakila_test`.`film_category`
...
comment on column `sakila`.`testtest`.`testcomment` is 'descriptionoffield';
...
create unique index `PRIMARY` on `sakila_test`.`city`(`city_id` asc);
create index `idx_fk_country_id` on `sakila_test`.`city`(`country_id` asc);
